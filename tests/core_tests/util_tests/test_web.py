...
def test_paste(mock_requests):
    mock_requests.add(
        mock_requests.POST, 'https://hastebin.com/documents', json={'key': 'foobar'}
    )
    from cloudbot.util import web

    assert (
        web.paste('test data', service='hastebin') == 'https://hastebin.com/foobar.txt'
    )
...
def test_paste_error(mock_requests):
    ...
    mock_requests.add(mock_requests.POST, 'https://hastebin.com/documents', status=502)
    assert web.paste('test data') == 'Unable to paste data'
...
def test_shorten(mock_requests):
    ...
    assert web.shorten('https://example.com', service='is.gd') == 'https://is.gd/foobar'

    assert web.Shortener().shorten('https://example.com') == 'https://example.com'
    ...
    assert (
        web.shorten('https://example.com', service='goo.gl') == 'https://goo.gl/foobar'
    )
...
def test_try_shorten(mock_requests):
    ...
    assert (
        web.try_shorten('https://example.com', service='is.gd') == 'https://example.com'
    )
...
def test_expand(mock_requests):
    ...
    assert web.expand('https://is.gd/foobar', service='is.gd') == 'https://example.com'
    ...
    mock_requests.replace(mock_requests.GET, 'http://my.shortener/foobar', status=404)
    with pytest.raises(web.ServiceHTTPError):
        web.expand('http://my.shortener/foobar')
    ...
    mock_requests.add(
        mock_requests.GET, 'https://www.googleapis.com/urlshortener/v1/url', status=404
    )
