...
def format_track(track, show_url=True):
    ...
    fmt = " - \x02{:,}\x02 plays, \x02{:,}\x02 favorites, \x02{:,}\x02 comments"
    out += fmt.format(
...
def format_user(user, show_url=True):
    ...
    fmt = (
        " - \x02{track_count:,}\x02 tracks, "
        "\x02{playlist_count:,}\x02 playlists, \x02{followers_count:,}\x02 "
        "followers, \x02{followings_count:,}\x02 followed"
    )
    out += fmt.format_map(user)
