...
NO_POLL = (
    "You have no current poll. Use .vote <user> <option> to vote on "
    "another users poll."
)

existing_poll_msg = (
    "You already have an active poll in this channel, "
    "you must close it before you can create a new one."
)
...
class Poll:
    ...
    def vote(self, voted_option, voter):
        """
        Adds a vote to a specific poll option. Raises PollError if
        option is invalid or user has already voted. Returns PollOption if
        sucessful.
...
def poll(text, conn, nick, chan, message, reply):
    """
    {<question>[: <option1>, <option2>[, <option3>]...|close} -
    Creates a poll for [question] with the provided options
    (default: Yes, No), or closes the poll if the argument is 'close'
    """
    # get poll ID
...
def vote(text, nick, conn, chan, notice):
    """
    <poll> <choice> - Vote on a poll; responds on error and silently
    records on success.
    """
